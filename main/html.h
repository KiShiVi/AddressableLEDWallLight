...
const char html_page[] =
    ...
    "    <script>\n"
    "\n"
    "		// ГЛАВНОЕ ДОБАВЛЕНИЕ: получаем токен из URL\n"
    "        const urlParams = new URLSearchParams(window.location.search);\n"
    "        const TOKEN = urlParams.get('token');\n"
    "\n"
    "        // Если токена нет — блокируем всё\n"
    "        if (!TOKEN) {\n"
    "            document.body.innerHTML = `\n"
    "                <h1 style=\"color: red; text-align: center; padding: 50px;\">\n"
    "                    Ошибка доступа\n"
    "                </h1>`;\n"
    "            throw new Error(\"Нет токена в URL\");\n"
    "        }\n"
    "\n"
    ...
    "		        // Функция отправки команды на ESP32\n"
    "        function sendCommand(commandName, command) {\n"
    "            const url = `/${commandName}?${command}&token=${TOKEN}`; // Формируем URL с командой\n"
    "\n"
    "            fetch(url)\n"
    "                .then(response => {\n"
    "                    if (!response.ok) {\n"
    ...
    "		// --- Функция для обновления состояния элементов на странице ---\n"
    "		function updateStatus() {\n"
    "			fetch(`/get_status?token=${TOKEN}`) // Запрашиваем текущий статус с сервера\n"
    "				.then(response => {\n"
    "                    if (!response.ok) throw new Error('Плохой токен');\n"
    "                    return response.json();\n"
    "                })\n"
    "				.then(data => {\n"
    "					// Обновляем состояние переключателей и слайдеров на основе полученных данных\n"
